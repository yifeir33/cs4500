...
public:
    ...
    void connect_to_server(){
        ...
        while(this->_check_for_socket_errors() == 0 && watchdog > 1000){
            ...
            if(_other_clients.length() >= EXPECTED && !_said_hello) {
                this->say_hello();
            }
    ...
    void say_hello(){
        ...
        for(size_t i = 0; i < _other_clients.length(); ++i){
            SockAddrWrapper *saw = dynamic_cast<SockAddrWrapper*>(_other_clients.get(i));
            assert(saw);
            Packet packet;
            packet.flags = FORWARD;
            packet.addrlen = saw->addrlen;
            packet.addr = saw->addr;
            memcpy(packet.value, hello.c_str(), hello.size());
            packet.vallen = hello.size() + 1;
            this->_send_packet(packet);
        }
        ...
        case ERROR_MSG:
            {
                ...
                char addr_str[INET_ADDRSTRLEN];
                inet_ntop(AF_INET,
                          &(packet.flags & FORWARD && packet.addrlen > 0 ? packet.addr.sin_addr : _server.sin_addr),
                          addr_str,
                          INET_ADDRSTRLEN);
                p(addr_str).p(":").p(ntohs((packet.flags & FORWARD && packet.addrlen > 0 ? packet.addr.sin_port : _server.sin_port))).p('\n');
                std::cout.flush();
                if(packet.flags & FORWARD) ++_received;
                break;
            }
            default:
